...
def test_duc_haba_api():
    """Test Duc Haba's API with the documented example"""
    ...
    try:
        logger.info("🧪 Testing Duc Haba's API with documented example...")
        ...
        logger.info("✅ Duc Haba API test successful")
        return True, result
    except Exception as e:
        error_msg = str(e)
...
def analyze_text():
    ...
    try:
        ...
        # Call Duc Haba's API using the documented endpoint
        try:
            logger.info("📡 Calling Duc Haba's /fetch_toxicity_level endpoint...")
            result = gradio_client.predict(
                msg=text_to_analyze,
                safer=safer_value,
                api_name="/fetch_toxicity_level"
            ...
            if "rate limit" in error_str.lower():
                return jsonify({
                    "error": "Duc Haba's API rate limit reached.",
                    "suggestion": "Please wait a few minutes before trying again.",
                    "compliance_note": "CLASS PROJECT: Only using duchaba/Friendly_Text_Moderation API"
                }), 429
            elif "timeout" in error_str.lower():
                return jsonify({
                    "error": "Connection to Duc Haba's API timed out.",
                    "suggestion": "The service may be busy. Please try again or use shorter text.",
                    "compliance_note": "CLASS PROJECT: Only using duchaba/Friendly_Text_Moderation API"
                }), 408
            elif "connection" in error_str.lower() or "failed to connect" in error_str.lower():
                return jsonify({
                    "error": "Failed to connect to Duc Haba's API.",
                    "suggestion": "The Gradio space may be sleeping or restarting. Please try again in a few minutes.",
                    "compliance_note": "CLASS PROJECT: Only using duchaba/Friendly_Text_Moderation API"
                }), 503
            else:
                return jsonify({
